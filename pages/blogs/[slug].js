...
const BlogDetail = ({ blog, preview }) => {
  ...
  return (
    <>
      <NextSeo
        title={`${blog.title} | nagisa-profile`}
        description={`${blog.summary}`}
        canonical={`https://nagisa-profile.com/blogs/${blog.slug}`}
        openGraph={{
          type: "article",
          url: `https://nagisa-profile.com/blogs/${blog.slug}`,
          title: `${blog.title} | nagisa-profile`,
          description: `${blog.summary}`,
          images: [
            {
              url: urlFor(blog.coverImage).height(600).url(),
              alt: `${blog.title}`,
            },
          ],
        }}
